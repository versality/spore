...
One spore worker (one task, one tmux session, one wt/<slug> branch)
...
the follow-up workers (M1-M3) that will land enforcement piece by
...
- The lifecycle of one worker from `draft` through `done`, including
...
- Coordinator-singleton lifecycle. This spec is per-worker only.

## 2. Vocabulary: macro state vs micro substate

The single load-bearing design call: split worker state into two
...
- The classifier is the single function that answers "what micro
  substate is this worker in right now?" - today that is
...
- Reject: history-replay, deterministic-replay engine. Overkill;
  spore workers are not durable workflows.
...
| `active`  | Worker owns the brief. Worktree + branch + tmux exist.     | `draft`, `paused`, `parked`, `blocked`          | `paused`, `parked`, `blocked`, `done`           | Worktree at `.worktrees/<slug>`, branch `wt/<slug>`, tmux session per `tmuxSessionName`.          | Counted against MaxWorkers.                         |
...
| `parked`  | Waits-for-named-trigger or external dependency. Worktree kept; intent: "wake when X happens". | `active` (worker or operator declares trigger)   | `active` (named trigger fires)                  | Worktree retained. Tmux reaped on idle, same policy as `paused`.                                  | Read by scheduler (trigger watcher); replenish skips. |
| `blocked` | Worker hit an environment / dependency wall. Worktree kept; intent: "I cannot proceed". | `active` (worker self-flip on permanent block)   | `active` (operator unblock), `done` (giving up) | Worktree retained. Tmux reaped on idle, same policy as `paused`.                                  | Skipped; surfaced in fleet status as a red row.     |
...
- `blocked` is distinct from `parked`: `blocked` means the worker has
  no path forward without operator help; `parked` means the worker
...
| `active.idle-unmerged`      | T: idle. G: clean tree, `rev-list main..wt/<slug>` > 0.        | Work shipped to a commit, not merged.             | **Stop hook fires (M1)**: tells the worker to run `wt merge`.     |
| `active.idle-unread`        | T: idle. I: unread > 0. W: no fresh marker.                    | A poke arrived; the worker has not woken to read.  | Fleet status surfaces; auto-wake (existing wakepending path).    |
| `active.merging`            | G: `MERGE_HEAD` or `rebase-merge` present.                     | Merge in flight; do not interrupt.                | Wait. No Stop-hook firing here.                                  |
| `active.blocked-permission` | T: pane shows a permission-prompt dialog (driver-specific regex). | Worker cannot proceed; harness-level wedge.        | Surface in fleet status; operator action required.              |
| `active.blocked-operator`   | T: idle. G: any. The brief has a question for the operator (heuristic: worker posted `wt ask`). | Worker is awaiting an operator answer.             | Surface in fleet status; do not auto-poke.                      |
| `active.dead`               | T: pane dead or zombie shell.                                  | Agent process gone.                                | Reconciler respawns; or operator marks `blocked`.               |

Substates are **observed**, not declared. There is no worker call
...
| E5  | `active`     | `parked`   | Worker or operator declares a named trigger               | Inbox empty                                                   | `flipStatus` to parked; record trigger in frontmatter (`trigger:` field, see sec 11) | `task.Park` (extend to take trigger name)           |
| E6  | `parked`     | `active`   | Named trigger fires (scheduler or operator)              | Trigger condition met                                         | Ensure session                                                                       | Scheduler / `task.Ensure`                           |
| E7  | `active`     | `blocked`  | Worker self-flip or operator command                      | Inbox empty                                                   | `flipStatus` to blocked; `reapIdleSlugSessions`; surface in fleet status              | `task.Block`                                        |
...
Numbered, falsifiable, each with a check site. The follow-up workers
...
- **I6.** Unmerged commits on `wt/<slug>` + clean tree + observed
  idle => Stop hook fires (M1). The worker receives a
...
follow-up worker that closes it.
...
| Prompt-blocking despite --danger | `active`            | dialog text in pane; worker wedged                 | `active`      | `active.blocked-permission`| Same as perm-mode regression                    | M2        |
...
- Wired as a Stop hook into the worker's claude settings via
  `bootstrap/skills/spore-worker-perm-default` (or its successor).
...
- `Request.CWD` (the worktree where the worker runs).
...
- `internal/fleet/liveness.classifyAgentPane` (or its lifted-out
  helper, see M1) to read the worker's own pane and decide if it
  is observed-idle.
...
- Does NOT actually run `wt merge`. The worker runs it, or refuses
...
- Does NOT distinguish "unit-end commit" from "intermediate
  ...
  the worker's behavior: if the worker is still working, the
  classifier sees `running` and the hook does not fire. If the
  worker stops mid-unit on purpose (e.g. brief is open-ended), the
  worker writes the next step and continues; the hook fires once
...
its own slug for a follow-up worker.
...
- Wire into claude worker settings via the existing perm-default
  skill or a sibling skill.
- Test: unit tests against a fake repo + fake tmux capture; end-
  to-end test on this very project (worker commits, stops, hook
  fires, worker runs `wt merge`, hook does not fire again).
...
modes visible in `fleet status` without changing worker behavior.
...
## 13. Done = follow-up workers can start
