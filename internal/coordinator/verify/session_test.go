...
func TestAnalyzeSessionCrossRepoPath(t *testing.T) {
	path := writeTranscript(t, []transcriptMsg{
		{
			...
			Message: struct {
				...
				Content: []struct {
					...
					{Type: "tool_use", Name: "Bash", Input: struct {
						Command string `json:"command,omitempty"`
					}{Command: "git -C /home/sky/projects/other-repo commit -m 'feat'"}},
				},
			},
		},
	})
	...
	if crossRepoPath == "" {
		t.Error("expected crossRepoPath to be set for git -C outside nix-config")
	}
}
...
func TestFindSessionFileFindsJSONL(t *testing.T) {
	...
	slug := "my-task"
	sessionDir := filepath.Join(projectsDir, "-home-sky-nix-config--worktrees-"+slug)
